...
let gameRooms = {};
const ADMIN_PASSWORD = "prada";

io.on('connection', (socket) => {
    console.log('A user connected:', socket.id);

    // Upgraded: Host creates game with business settings
    socket.on('createRoom', (roomId, password, businessConfig) => {
        if (password === ADMIN_PASSWORD) {
            socket.join(roomId);
            
            let ticketPool = [];
            for(let i = 1; i <= businessConfig.ticketCount; i++) {
            ...
            gameRooms[roomId] = {
                host: socket.id,
                config: businessConfig, // Stores Date, Price, Prize Money!
                drawnNumbers: [],
                availableNumbers: Array.from({ length: 90 }, (_, i) => i + 1),
                players: {},
                patterns: businessConfig.patterns,
                winners: {},
                tickets: ticketPool
            };
            socket.emit('roomCreatedSuccess', businessConfig);
        } else {
    ...
    socket.on('requestTickets', (roomId, playerName) => {
        ...
        if (room) {
            ...
            // Now we also send the ticket price to the player!
            socket.emit('showTicketPool', availableTickets, playerName, room.config.ticketPrice);
        } else {
    ...
    socket.on('changeTheme', (roomId, themeData) => {
        let room = gameRooms[roomId];
        if (room && room.host === socket.id) { io.to(roomId).emit('applyTheme', themeData); }
    });
});
